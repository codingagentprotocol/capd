...
data validation page, use `capd console --probe`. The command passes a
short-lived scoped browser token to the page without printing it to the
terminal; the root daemon token remains in `~/.capd/token` for CLI clients. The
probe also calls
...
capd run --agent codex --account auto --profile work "..." # route inside a named account profile
...
clients. The raw daemon token is still full-scope for backward compatibility;
console and probe URLs generated by `capd console` use signed, short-lived
`console` or `probe:read` scoped tokens. `console` is the interactive local
cockpit scope for account diagnostics plus session task control; `probe:read`
stays diagnostic-only. First call must be `initialize` (version negotiation).
...
- Token auth on every connection (`~/.capd/token`, 0600, generated on first
  run); browser URLs use signed scoped tokens derived from it.
...
- New Codex sessions can opt into an imported account with `--account` or
  ...
  `interactive`, or `vision`); `capd run` and the Web Console infer a
  conservative task class from the prompt when task intent is left on automatic.
  Long-running and review tasks penalize stale or missing quota, while
  interactive tasks increase recent-failure penalty.
...
- Account-aware routing policy: `internal/account.RoutePolicy` now centralizes
  ...
  deterministic. The Web Console exposes task intent controls for route preview
