...
| Account-aware routing | `capd agents route --account auto --require-fresh-quota --json`, `session/create.accountId:"auto"`, route candidates, route policy, selected account reason, recent failure penalty evidence | `make verify-codex-readiness-sim` covers deterministic score ordering, stale/missing quota conservatism, profile filters, recent failure penalty, fresh-quota failure evidence, and route-candidate redaction | `capd agents route --account auto --profile <name> --require-fresh-quota --json` after fresh live quota | Implemented with conservative quota-pressure policy plus account health penalty; task-class scoring is next evolution |
...
- Add configurable scheduler weights for account health penalty, task class, and
