...
| `capd probe data [--json] [--readiness] [--fail] [--require-secret-backend <file\|native>]` | fetches `/probe/data` with `Authorization: Bearer <daemon-token>` and prints safe diagnostics for automation. `--readiness` requests the stronger readiness view, `--require-secret-backend` passes the expected backend to the daemon, and `--fail` exits non-zero when the probe reports `ok=false` or an HTTP error status. |
