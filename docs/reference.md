...
| `capd agents usage codex --account <id\|auto>` | usage for an imported Codex account, or the lowest cached-quota Codex account with `auto`; also refreshes the local quota snapshot |
...
| `capd accounts codex smoke [--json] [--quota]` | Verify imported accounts, SecretStore readability, per-account projection, auth file permissions, and optionally quota refresh without printing token material. |
