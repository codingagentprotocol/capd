...
Credential failures also use the safe per-account `secretState` enum to choose
the next step, including direct `capd secretstore check --json --roundtrip`
guidance for generic `unreadable` SecretStore failures.
...
Use `--prompt-free` for a fast metadata/quota/routing preflight that skips
per-account SecretStore credential reads; it marks credential evidence as
`not-checked` and cannot be combined with `--verify-secretstore`.
...
| `--profile` | — | with `--account auto`, constrain Codex account selection to a named account profile |
...
| `capd secretstore check [--json] [--roundtrip] [--secret-backend <file\|native>] [--require-backend <file\|native>] [--timeout 2m]` | opens the selected SecretStore backend and prints safe readiness evidence. `--roundtrip` writes, reads, and deletes a diagnostic secret; `--require-backend` fails when the active backend differs, making this the smallest direct native SecretStore gate before live account checks. `--timeout` bounds OS credential backend waits, including native prompts. Failed native roundtrips return safe platform-specific next steps for macOS Keychain access, Linux Secret Service/`secret-tool`, or Windows Credential Manager without embedding credential material or command output. |
| `capd console [--probe] [--url] [--require-secret-backend <file\|native>]` | opens the local web console, or the compact validation probe with `--probe`, after checking daemon health. By default it passes a signed short-lived scoped token to the browser without printing it; `--url` prints that scoped tokenized URL only when explicitly requested. `--probe` uses `probe:read`; the full console uses `console`, which permits bundled console account/diagnostic workflows and local session task control. `--require-secret-backend` preloads the page's readiness gate with the selected SecretStore backend so browser checks match CLI readiness runs. The full console can export a prompt-free browser evidence package with inline health, probe, route, and account-check artifacts for local QA handoff. |
| `capd probe data [--json] [--readiness] [--fail] [--require-secret-backend <file\|native>] [--timeout 2m]` | fetches `/probe/data` with `Authorization: Bearer <daemon-token-or-scoped-token>` and prints safe diagnostics for automation. Text output includes a compact readiness summary with route candidate count, actual/required SecretStore backend evidence, route-candidate `secretBackend` enums when present, the active `routePolicy` summary when returned, and server JSON errors when available. Without `--readiness`, the endpoint returns `promptFree:true`, labels text output as prompt-free account metadata, and uses cached account metadata plus route evidence without reading account credentials. `--readiness` requests the stronger readiness view and defaults the daemon request to `requireSecretBackend=native`; use `--require-secret-backend file` only for intentional file-backend tests. JSON includes a safe `routeHistory` array projected from recent `agents.route` audit events plus `summary.routeHistoryEvents`; rows contain only time, actor, outcome, agent, account id, account mode, profile, task class, quota state/freshness, and route-candidate count. `--timeout` bounds HTTP waits, and `--fail` exits non-zero when the probe reports `ok=false` or an HTTP error status. |
| `capd probe evidence --manifest <manifest.json\|summary.json> [--artifact <file>]... [--json] [--html <file>] [--fail]` | validates saved live selftest evidence without contacting the daemon. With only `--manifest`, it follows artifact paths from the manifest or summary; relative artifact paths are resolved from the manifest directory, and explicit `--artifact` flags override that list. The report extracts safe route policy, route decision status, route candidate count, fresh quota evidence, repair-plan count, optional audit metadata, SecretStore backend, daemon mode, and selftest status, and emits a `checks` array for CI/Web consumers. If an `audit.json` artifact is present, the report counts events and fails unsafe audit evidence containing access/refresh tokens, SecretStore refs, raw auth fields, credential strings, or local paths. `--html` writes a standalone QA report from the same safe summary without embedding raw artifact JSON. `--fail` exits non-zero when the saved package lacks passed status, backend, daemon mode, routePolicy, routeDecisionOk, routeCandidates, fresh quota evidence, or when included audit metadata is unsafe. |
| `capd support bundle [--out dir] [--require-secret-backend <file\|native>] [--probe-data=false] [--timeout 2m] [--fail]` | collects a redacted local evidence package for debugging and CI handoff. The bundle writes `manifest.json`, `doctor-prompt-free.json`, `agents-route.json`, `audit.json`, `health.json`, optional `probe-data.json`, and generated `report.html` into one directory, then validates the package with the same `probe evidence` checks. Collection is prompt-free by default: it does not read per-account SecretStore credentials, and daemon/probe failures are captured as safe JSON artifacts instead of aborting the bundle. The audit artifact includes recent safe event metadata only; token material, SecretStore refs, raw auth payloads, and local paths are dropped before export. `--fail` exits non-zero only after the package is written and the generated evidence report is not ready. |
| `GET /probe/data` | authenticated HTTP diagnostics endpoint for Web clients and smoke tests. Requires `Authorization: Bearer <daemon-token>`. The ordinary path returns safe JSON health, a compact `summary`, cached account metadata, `agents/route`, `routeCandidates`, `routePolicy`, safe recent `routeHistory`, and pass/fail `checks` without reading SecretStore credentials or projecting runtimes. `?readiness=1` defaults to `requireSecretBackend=native` for the stronger live readiness view and adds the daemon-side `accounts/check` gate; `?readiness=1&requireSecretBackend=file` is reserved for intentional file-backend tests. `requireSecretBackend` accepts only `file` or `native`; unknown values fail fast with HTTP 400 before quota or route checks run. The top-level `summary` reuses the account summary shape for account, quota, auto-route, and SecretStore evidence, then adds route-decision status and `routeHistoryEvents` for the full Web diagnostics path. The handler also bounds server-side work: ordinary probes get 12s and readiness probes get 2m. |
| `capd doctor [--json] [--fail] [--prompt-free] [--verify-secretstore] [--require-secret-backend <file\|native>] [--timeout 2m]` | local readiness preflight for daemon health, Codex CLI availability, imported account count, per-account SecretStore credential readability, quota freshness, auto-route freshness, daemon-side CAP account evidence, and SecretStore backend; `--prompt-free` skips per-account SecretStore reads for fast metadata/quota/routing status; `--verify-secretstore` performs an explicit write/read/delete diagnostic roundtrip and cannot be combined with `--prompt-free`; `--timeout` bounds native SecretStore and account checks; text mode fails when issues are found, and `--fail` makes JSON mode fail too |
| `capd agents list` | table: id, available/not installed, version, binary path |
| `capd agents route [--account <id\|auto>] [--profile name] [--capability name] [--require-fresh-quota] [--json]` | preview local routing without starting a session; with `--account auto`, shows the Codex account selected by conservative quota scoring. `--profile` constrains auto account selection and `routeCandidates` to a named account profile, which is useful for work/personal/CI account pools. JSON includes `routeCandidates`, sorted by the same account-aware routing score, plus safe `routePolicy` evidence. Text output includes the active route policy when account routing is used. `--require-fresh-quota` fails unless that auto selection is backed by fresh cached quota; failing text output includes the selected route, route policy, and route-candidate quota evidence so users can see stale or missing quota before refreshing. With `--json`, fresh-quota failures also print `{"ok":false,"error", "data", "nextSteps"}` with safe `accountRoute`, `routeCandidates`, `routePolicy`, and `secretBackend` evidence before exiting non-zero |
...
| Command | Meaning |
|---------|---------|
| `capd accounts list [--json]` | List imported account metadata across all providers; provider-scoped current accounts are marked with `*`; quota state is reported as `fresh`, `stale`, or `missing`. JSON includes safe `quotaFresh`, and Codex rows include account-aware `routeScore`, `routeReason`, `recentFailures`, `lastFailureAt`, and `healthPenalty` evidence without reading SecretStore token material. |
| `capd accounts profile create <name> [--provider codex] [--description text]` | creates or updates a provider-scoped account profile. Profiles are SQLite metadata only; token bundles remain in SecretStore. |
| `capd accounts profile add <name> <account-id>...` / `remove <name> <account-id>...` | manages safe account membership for a profile. Accounts must belong to the profile provider, so a Codex profile cannot accidentally include another provider's credentials. |
| `capd accounts profile list [--json]` / `show <name> [--json]` / `current [name]` | lists, inspects, or selects the current account profile. JSON output includes safe account summaries such as email, plan, SecretStore backend enum, quota state, and route score, but never SecretStore refs or token material. |
| `profiles/list`, `profiles/update`, `profiles/members` RPC | Web/desktop-safe profile management over CAP. `profiles/list` returns profile metadata and optional safe member account summaries; `profiles/update` creates, updates, deletes, or selects a current profile; `profiles/members` adds/removes accounts. These methods never return SecretStore refs, auth JSON, token material, or runtime paths. |
| `capd accounts import [--provider codex] [--auth path ...] [--json]` | Requires a running daemon (`capd start`). Imports one or more Codex `auth.json` files through daemon `accounts/import`, matching the CAP/WebSocket path used by web clients. Repeat `--auth` to import multiple explicit paths; without `--auth`, `CAPD_CODEX_AUTH_PATHS` can provide an OS path-list of auth files (`:` on macOS/Linux, `;` on Windows), otherwise the daemon uses its default `~/.codex/auth.json`. The bundled Web Console accepts multiple auth paths separated by newlines or commas. |
| `capd accounts check [--provider codex] [--json] [--readiness] [--refresh-quota] [--require-multiple] [--require-fresh-quota] [--require-all-fresh-quota] [--require-secret-backend <file\|native>] [--timeout 2m]` | Requires a running daemon (`capd start`). Optionally refresh every imported Codex quota through daemon `accounts/quota`, then call the daemon's `accounts/check` RPC and print safe smoke evidence without token material or runtime paths. JSON includes a compact `summary`, per-account quota/credential/runtime checks, safe recent-failure health evidence, and `routeCandidates` when imported accounts are available, using the same ordering as `agents/route --account auto`; failed readiness evidence also includes the shared `repairPlan` with `execution.runnable` and `execution.reason`, so CAP clients can consume the same remediation contract as `/probe/data` and `capd repair run --json`. Failing JSON calls also print structured `ok:false` errors and safe `nextSteps`, including macOS Keychain `access-denied` recovery when native SecretStore access is canceled and direct `capd secretstore check --json --roundtrip` guidance for generic `unreadable` SecretStore failures. Use `capd accounts codex smoke` for direct local checks that do not need the daemon. `--readiness` is the recommended daemon-side gate for live Codex work: it enables quota refresh, multiple-account, fresh auto-route, all-fresh quota, and native SecretStore requirements by default. `--timeout` bounds daemon-side waits so live preflight does not hang on remote quota refresh. `--require-secret-backend` accepts only `file` or `native` and can override the readiness backend for intentional file-backend tests. |
| `accounts/quota` RPC with `accountId:"all"` | Daemon-side batch quota refresh used by Web Console and readiness gates. On mid-batch failure, the JSON-RPC error `data` includes safe partial `accounts` evidence for accounts already refreshed, plus `failedAccount` and runnable `nextSteps`, without token material or raw backend usage JSON. |
...
| `capd accounts codex migrate-secrets [account-id\|all] [--from file\|native] [--to file\|native] [--dry-run] [--delete-source] [--json] [--timeout 2m]` | Move imported Codex account credentials between SecretStore backends and update safe account metadata refs without printing token material. Defaults to `--from file --to native` for upgrading older imports to the OS SecretStore. The target secret is read back before account metadata is updated; if target readback fails, capd removes the attempted target secret, keeps the source ref, and reports safe partial evidence. Successful migrations print a target-backend readiness command such as `CAPD_SECRET_BACKEND=native capd accounts check --json --readiness --require-secret-backend native --timeout 2m`; by default the source secret is kept as a rollback path, so add `--delete-source` only after native readiness passes. `--dry-run` reports planned account moves without writing target secrets. |
| `capd accounts codex quota [account-id\|auto\|all] [--raw] [--timeout 2m]` | Fetch ChatGPT backend quota for imported Codex accounts and update local quota snapshots. `auto` uses the same conservative quota scoring rule as account-aware routing. `all` refreshes every imported Codex account and prints safe summaries; if one account fails after earlier successes, it prints safe `ok:false` partial JSON with the already refreshed summaries, failed account id, scrubbed error, and next steps. Defaults to a safe summary; `--raw` prints backend usage JSON for debugging and is only supported for a single account. `--timeout` bounds SecretStore reads and remote quota waits. |
| `capd accounts codex smoke [--json] [--quota] [--require-fresh-quota] [--require-all-fresh-quota] [--require-secret-backend <file\|native>] [--timeout 2m]` | Verify imported accounts, SecretStore readability, per-account projection, auth file permissions, auto-route account selection, and optionally quota refresh without printing token material. JSON includes `autoRoute.accountId`, `autoRoute.quotaState` as `fresh`, `stale`, or `missing`, and `routeCandidates` sorted by the same account-aware routing score, plus per-account `secretChecked`, `runtimeChecked`, `secretBackendOk`, `secretReadable`, `secretState`, `quotaState`, `quotaFresh`, and `quotaCheckedAt`; early prompt-free failures set the checked booleans to `false` so cached evidence is not confused with a failed SecretStore or runtime check. With `--json`, failing gates and SecretStore read failures still print safe partial evidence with `ok:false`, `issues`, and `nextSteps`. When partial evidence already proves additional repair work, such as stale auto-route quota or SecretStore backend mismatch, `nextSteps` includes those safe parallel repairs too. Text output prints the same safe per-account secret/runtime/quota columns plus auto-route and route-candidate quota evidence for quick terminal checks. `--timeout` bounds SecretStore reads, runtime projection, and optional quota refresh. `--require-fresh-quota` fails unless auto-route selection is backed by fresh cached quota; `--require-all-fresh-quota` fails unless every imported account has fresh cached quota; `--require-secret-backend` fails unless the active SecretStore backend matches. |
...
local clients. The raw daemon token is `full` scope; signed `console`,
`console:read`, and `probe:read` tokens are accepted by bundled browser pages.
`console` is the interactive local cockpit scope and may create, continue, and
cancel sessions; `console:read` and `probe:read` remain diagnostic scopes that
reject task-control methods. The bundled Console and Probe pages accept a query
token only as a bootstrap path: after reading it they remove `token` from the visible URL
with `history.replaceState`, send WebSocket auth via the
`capd.auth.*`
