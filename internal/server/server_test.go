...
func TestConsoleStaticContract(t *testing.T) {
	...
	required := []string{
		...
		"刷新全部 quota",
		"quota 批量刷新完成",
		`call("accounts/quota", { provider: "codex", accountId: account.id })`,
		"importCodexAccount",
		"导入 Codex",
		"checkAccounts",
		"检查账号",
		...
		"native secret",
		"assertAccountsReady",
		"expected multiple Codex accounts",
		"auto route does not have fresh cached quota",
		`result.secretBackend !== "native"`,
		"row.quotaFresh",
		"setCurrentAccount",
		"设为当前",
		"projectAccount",
