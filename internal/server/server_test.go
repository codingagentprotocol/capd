...
func TestProbeDataReturnsSafeAccountRouteEvidence(t *testing.T) {
	...
	if len(got.RouteHistory) != 1 || got.RouteHistory[0].AccountID != "codex-test" || got.RouteHistory[0].Outcome != "ok" || got.RouteHistory[0].Actor != "daemon" || got.RouteHistory[0].QuotaState != protocol.AccountQuotaStateFresh || !got.RouteHistory[0].QuotaFresh {
		t.Fatalf("routeHistory = %+v", got.RouteHistory)
	}
	...
	if check := probeCheckByName(got.Checks, "route audit history"); !check.OK || !strings.Contains(check.Evidence, "1 events") || !strings.Contains(check.Evidence, "latest codex-test") {
		t.Fatalf("route audit history check = %+v", check)
	}
...
func TestConsoleStaticContract(t *testing.T) {
	...
	required := []string{
		...
		"导出 Evidence",
		"capd-web-console-evidence",
		"web-console-evidence",
		"inline:health.json",
		"inline:probe-data.json",
		"inline:agents-route.json",
		"inline:accounts-check.json",
		"downloadJSON",
		"redactEvidence",
		"packageStatus",
		"lastHealthInfo",
		...
		`document.getElementById("taskClass").onchange = previewRoute`,
		"res.accountId",
